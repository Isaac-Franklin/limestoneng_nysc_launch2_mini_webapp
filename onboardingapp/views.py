...
def HomePage(request):    
    if request.method == 'POST':
        ...
        if checkEmail:
            ...
            referralURL = 'https://nysc.limestone.ng/referralcode'
            login(request, user)
    ...
    try:        
        ...
        for registrant in getAllRegistrants:
            if registrant.referralcode:
                ...
                getAllReferrer = LaunchRegistrants.objects.get(referralcode = getRegistrantReferrerCode)
        ReferralRecords.objects.filter(email = getAllReferrer.email).delete()
        form = ReferralRecords(fullname = getAllReferrer.fullname,email = getAllReferrer.email,  numberOfReferrals = getAllRegistrantReferralsCount)
        form.save()
